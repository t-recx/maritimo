...
describe Accumulator do
  ...
  describe :publish do
    ...
    describe "when messages are of multi-sentence type" do
      it "should only be sent when completed" do
        subject.publish queue, "!AIVDM,2,1,6,B,533>H`42;mh@=<8f220IE8p5>22222222222221?<@25=6V;0B4;`11C1hA0,0*3C", logger
        _(queue.published.length).must_equal(0)
        _(subject.bag.length).must_equal(1)
        subject.publish queue, "!AIVDM,2,2,6,B,H8888888880,2*51", logger
      ...
      describe "and when sending messages with various mixes of ip, source id or both" do
        it "should only be sent when completed - test case #1" do
          ...
          _(queue.published.length).must_equal(0)
          _(subject.bag.length).must_equal(1)
          subject.publish queue, "[100.29.29.1]!AIVDM,2,1,6,B,533>H`42;mh@=<8f220IE8p5>22222222222221?<@25=6V;0B4;`11C1hA0,0*3C", logger
          _(queue.published.length).must_equal(0)
          _(subject.bag.length).must_equal(2)
          subject.publish queue, "\\s:203*22\\!AIVDM,2,1,6,B,533>H`42;mh@=<8f220IE8p5>22222222222221?<@25=6V;0B4;`11C1hA0,0*3C", logger
          _(queue.published.length).must_equal(0)
          ...
          subject.publish queue, "\\s:203*22\\!AIVDM,2,2,6,B,H8888888880,2*51", logger
          ...
          subject.publish queue, "[100.29.29.1]!AIVDM,2,2,6,B,H8888888880,2*51", logger
          _(subject.bag.length).must_equal(0)
        ...
        it "should only be sent when completed - test case #2" do
          ...
          subject.publish queue, "!AIVDM,2,1,7,A,54R?P2029SIAK8@6221LUA@Dp8E8LDr2222222169`;496T40>0PDTQBDSp8,0*65", logger

          _(queue.published.length).must_equal(0)
          _(subject.bag.length).must_equal(2)

          subject.publish queue, "[100.100.2.2]!AIVDM,2,2,6,B,H8888888880,2*51", logger # should be discarded, since it starts on fragment = 2

          _(queue.published.length).must_equal(0)
          _(subject.bag.length).must_equal(2)

          subject.publish queue, "[2.1.1.2]!AIVDM,2,1,8,B,53sO7A02Bu:@=<L;:21<4<u:0TV222222222221@7P?9540Ht00000000000,0*04", logger
          ...
          subject.publish queue, "[2.1.1.2]!AIVDM,2,2,8,B,00000000000,2*2F", logger

          _(queue.published.length).must_equal(2)
          _(subject.bag.length).must_equal(1)

          subject.publish queue, "\\s:200*00\\!AIVDM,2,2,7,A,88888888880,2*23", logger

          _(queue.published.length).must_equal(2)
          _(subject.bag.length).must_equal(1)

          subject.publish queue, "\\t:43298,s:4039,x:34043*00\\!AIVDM,2,1,1,A,53btkN02>8da0P@6220DhU@F2222222222222217AH::<6D60@T532Dp8888,0*2A", logger

          _(queue.published.length).must_equal(2)
          _(subject.bag.length).must_equal(2)

          subject.publish queue, "\\t:43298,s:4039,x:34043*00\\!AIVDM,2,2,1,A,88888888880,2*25", logger

          _(queue.published.length).must_equal(3)
          _(subject.bag.length).must_equal(1)

          subject.publish queue, "!AIVDM,2,2,7,A,88888888880,2*23", logger
    ...
    describe "when message is sent starting from the middle of the sentence" do
      it "should not be stored nor published" do
        subject.publish queue, "!AIVDM,3,2,7,A,88888888880,2*23", logger

        _(queue.published.length).must_equal(0)
        _(subject.bag.length).must_equal(0)

        subject.publish queue, "!AIVDM,3,3,7,A,88888888880,2*23", logger
    ...
    describe "when message fragment is sent and when there was already a stored fragment for the message" do
      it "should not be stored nor published - test case #1" do
        subject.publish queue, "!AIVDM,3,1,7,A,88888888880,2*23", logger

        _(queue.published.length).must_equal(0)
        _(subject.bag.length).must_equal(1)

        subject.publish queue, "!AIVDM,3,1,7,A,88888888880,2*23", logger # First message: reset it
    ...
    describe "when messages are invalid" do
      it "should be discarded" do
        subject.publish queue, "!AIVDM,1,1,,A,13sO4ugP00OF8;nF8V:TBwvB00S<,0*22", logger
        subject.publish queue, "INVALID MESSAGE", logger
        subject.publish queue, "!AIVDM,2,1,6,B,533>H`42;mh@=<8f220IE8p5>22222222222221?<@25=6V;0B4;`11C1hA0,0*3C", logger
        subject.publish queue, "!INVALID,MESSAGE,xxx,1,fdjsfkljdsklfjdsklj", logger
        subject.publish queue, "[3043.324.243249.3423]\\s:34892094*22\\vxnmore,1,1,2,B,fkdjfskjdskfj,2*99", logger
        subject.publish queue, "!AIVDM,2,2,6,B,H8888888880,2*51", logger
        subject.publish queue, "430294fkdjsjeqcdnms dfkjs fdsncdksn !AIVDM fkdljsfljdsvn", logger
        subject.publish queue, "!AIVDM,,,,B,H8888888880,2*51", logger
        subject.publish queue, "!AIVDM,,1,1,B,H8888888880,2*51", logger
  ...
  def exercise_immediate_publish
    ...
    messages.each do |message|
      ...
      subject.publish queue, message, logger
