...
pub fn get_number_from_payload(payload: &str) -> Result<usize, NMEADecoderError> {
...
pub fn get_signed_number_from_payload(payload: &str) -> Result<isize, NMEADecoderError> {
    let value_bits = &payload[1..payload.len()];

    match get_number_from_payload(&payload[0..1]) {
        Ok(sign_bit) => {
            if sign_bit == 0 {
                match get_number_from_payload(value_bits) {
