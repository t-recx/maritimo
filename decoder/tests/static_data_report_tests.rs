...
fn decode_when_message_part_a_and_message_size_incorrect_should_return_error() {
    let result = decoder::decode("!AIVDM,1,1,,A,H42O55i18tMET0000000000000,2*6D", &mut HashMap::new()).unwrap_err();
...
fn decode_when_message_part_b_and_message_size_incorrect_should_return_error() {
    let result = decoder::decode("!AIVDM,1,1,,A,H42O55lti4hhhilD3nink000?05,0*40", &mut HashMap::new()).unwrap_err();
...
fn decode_should_decode_static_data_report_part_a() {
    let message = decoder::decode("!AIVDM,1,1,,A,H42O55i18tMET00000000000000,2*6D", &mut HashMap::new()).unwrap().unwrap();
    ...
    match message.data {
        MessageData::StaticDataReportPartA { 
            vessel_name
        } => {
...
fn decode_should_decode_static_data_report_part_b() {
    let message = decoder::decode("!AIVDM,1,1,,A,H42O55lti4hhhilD3nink000?050,0*40", &mut HashMap::new()).unwrap().unwrap();
