...
fn decode_when_message_size_incorrect_should_return_error() {
    let result = decoder::decode("!AIVDM,1,1,,A,403OviQuMGCqWrRO9>E6fE700@G,0*4D", &mut HashMap::new()).unwrap_err();
...
fn decode_should_decode_base_station_report() {
    let message = decoder::decode("!AIVDM,1,1,,A,403OviQuMGCqWrRO9>E6fE700@GO,0*4D", &mut HashMap::new()).unwrap().unwrap();
